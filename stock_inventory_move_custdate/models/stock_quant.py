...
class StockQuant(models.Model):
    ...
    def _compute_is_outdated(self):
        ...
        for quant in self.filtered(
            lambda x: x.product_uom_id and float_compare(
                x.quantity_at_date,
                x.quantity,
    ...
    def _apply_inventory(self):
        ...
        """        
        # Prevent inventory adjustment quant that will be generated to be
        #  linked to inventory adjustment
        self = self.with_context(
            default_to_do=False, default_current_inventory_id=False
        )

        # ---------------------------------------------------------------------
        # Alternative code using current_invetory_id field
    ...
    def _get_inventory_fields_write(self):
        return super()._get_inventory_fields_write() + ["current_inventory_id"]

    @api.model
    def create(self, vals):
        """
        TODO this is not covered by OCA's stock inventory, so we must add it
        """
        ret = super().create(vals)
        if ret.current_inventory_id:
            ret.current_inventory_id.stock_quant_ids |= ret
        return ret
